...
namespace YingLong
{
    ...
    public:
        ...
        // ========================================================================
        // Light Culling (Tile-Based) - Delegated to DX12LightCullingManager
        // 光源剔除（Tile-Based）— 委托给 DX12LightCullingManager
        // ========================================================================

        /**
         * @brief 获取光源剔除管理器 / Get light culling manager
         * @return 光源剔除管理器指针 / Light culling manager pointer
         */
        DX12LightCullingManager* GetLightCullingManager() const noexcept { return pLightCulling.get(); }

        /**
         * @brief 检查光源剔除资源是否就绪 / Check if light culling resources are ready
         * @return 是否就绪 / Whether ready
         */
        bool IsLightCullingReady() const noexcept { return pLightCulling && pLightCulling->IsReady(); }
    ...
    private:
        ...
        // Light culling manager (Tile-Based) / 光源剔除管理器（Tile-Based）
        std::unique_ptr<DX12LightCullingManager> pLightCulling;
